...
public class EmailableReporterListener implements IReporter{
    ...
    private String dingoMergeBranch = CommonArgs.getMergeBranch();
    ...
    public void generateReport(
        ...
        String dateNowStr = CommonArgs.getCurDateStr("yyyyMMdd");

        if (! new File("./dailyreport/" + dateNowStr).exists()) {
            if (! new File("./dailyreport/" + dateNowStr).mkdirs()) {
    ...
    protected void writeHead() {
        ...
        writer.println("<title>DingoDB每日测试报告</title>");
        writeStylesheet();
        writer.println("</head>");
    }
    ...
    protected void writeStylesheet() {
        ...
        writer.print(".skippedeven td,.stripe .atts {background-color: #FB1}");
    ...
    protected void writeBody() {
        String dateNowStr = CommonArgs.getCurDateStr("yyyy-MM-dd HH:mm:ss");
        writer.println("<body>");
        ...
        writer.println("<h4>. 本次构建的触发分支：" + dingoMergeBranch + "</h4>");
...
//        writeScenarioDetails();
    ...
    protected void writeSuiteSummary() {
        ...
        if (testIndex > 1) {
            ...
            writeTableHeader(
                integerFormat.format(totalSkippedTests), (totalSkippedTests > 0 ? "num atts" : "num"));
            writeTableHeader(
        ...
        writer.println("</table>");
        writer.println("*注：Skipped表示本次测试被标记不需要执行的用例或者由于数据库连接失败导致，并不是由于用例间依赖原因导致的跳过");
        writer.println("<br>");
        writer.println("<br>");
        String hrefUrl = "http://172.20.61.7/dingo/auto_tests/sql_report/" + CommonArgs.getCurDateStr("yyyyMMdd") + "/";
