...
public class EmailableReporterListener implements IReporter{
    ...
    public void generateReport(
        ...
        if (! new File("./dailyreport/" + dateNowStr).exists()) {
            ...
            try {
                writer = new PrintWriter(newBufferedWriter(new File("./dailyreport/" + dateNowStr + "/", fileName).toPath(), UTF_8));
            } catch (IOException e) {
            ...
            try {
                File file = new File("./dailyreport/" + dateNowStr + "/", fileName);
                if (file.exists()) {
                    file.delete();
                    writer = new PrintWriter(newBufferedWriter(new File("./dailyreport/" + dateNowStr + "/", fileName).toPath(), UTF_8));
                }
        ...
        try {
            Thread.sleep(3000);
            sendEmailClient.sendHTMLEmail("./dailyreport/" + dateNowStr + "/" + fileName);
